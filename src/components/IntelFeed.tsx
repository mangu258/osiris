...
function getRiskLabel(score: number): string {
  if (score >= 8) return '危急';
  if (score >= 6) return '高';
  if (score >= 4) return '升高';
  return '低';
}
    ...
    >
      ...
      >
        <div className="flex items-center gap-2">
          ...
          <span className="hud-text text-[12px] text-[var(--text-primary)]">情报源</span>
          ...
          {news.some((n: any) => n.risk_score >= 8) && (
            <span className="gotham-tag gotham-tag--critical" style={{ fontSize: '7px', padding: '1px 4px' }}>警报</span>
          )}
        </div>
        <div className="flex items-center gap-2">
